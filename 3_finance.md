...
- **NPV (Net Present Value) rule** — invest when NPV > 0; cash flows at different times are not comparable without discounting
- **CAPM (Capital Asset Pricing Model)** — r = rᶠ + β(E[rₘ] − rᶠ); required return given risk
- **WACC (Weighted Average Cost of Capital)** — blended cost of capital; use as discount rate for project evaluation
- **3 statements** — Income Statement (flow), Balance Sheet (snapshot), Cash Flow (sources & uses)
- **Net Income ≠ Cash Flow** — always check cash flow from operations (CFO); profitable companies can go bankrupt
...
Gross Profit − OpEx (Operating Expenses) = EBIT (Earnings Before Interest & Taxes)
...
Current assets: Cash, AR (Accounts Receivable), Inventory
Non-current: PP&E (Property, Plant & Equipment), Intangibles
Current liabilities: AP (Accounts Payable), Accrued expenses, Short-term debt
...
**Efficiency:**
- DSO (Days Sales Outstanding) = AR / (Revenue/365) — days to collect from customers
- DIO (Days Inventory Outstanding) = Inventory / (COGS/365) — days inventory sits
- DPO (Days Payable Outstanding) = AP / (COGS/365) — days before paying suppliers
- **CCC (Cash Conversion Cycle) = DSO + DIO − DPO** — days cash is tied up (Amazon/Walmart run negative CCC — customers fund the business)
